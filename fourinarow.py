...
def drawBoard(board, extraToken=None, turn=None):
    ...
    if turn is not None:
        ...
        turn_text = "Your Turn (Red)" if turn == HUMAN else "Computer's Turn (Black)"
        text = font.render(turn_text, True, TEXTCOLOR)
            ...
            if event.type == QUIT:
                ...
                if tokeny < YMARGIN and tokenx > XMARGIN and tokenx < WINDOWWIDTH - XMARGIN:
                    ...
                    if isValidMove(board, column):
                        animateDroppingToken(board, column, RED, turn=HUMAN)
...
def animateDroppingToken(board, column, color, turn=None):
    ...
    while True:
        ...
        if int((y - YMARGIN) / SPACESIZE) >= lowestEmptySpace:
            return
        drawBoard(board, {'x':x, 'y':y, 'color':color}, turn=turn)
        pygame.display.update()
        FPSCLOCK.tick()
...
def animateComputerMoving(board, column):
    ...
    animateDroppingToken(board, column, BLACK, turn=COMPUTER)
